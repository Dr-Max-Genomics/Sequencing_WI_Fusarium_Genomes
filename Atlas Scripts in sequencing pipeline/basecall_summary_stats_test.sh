#!/bin/bash

#SBATCH --job-name="summ_basecall"   	   #name of this job
#SBATCH -A silage_microbiome		   #name of account
#SBATCH -p atlas
#SBATCH --qos=normal		 	   #Quality of service for the job
#SBATCH -n 40                   	   #number of logical cores
#SBATCH -t 24:00:00        		   #time allocated for this job hours:mins:seconds
#SBATCH --mail-user=maxwell.chibuogwu@usda.gov   #enter your email address to receive emails
#SBATCH --mail-type=BEGIN,END,FAIL #will receive an email when job starts, ends or fails
#SBATCH -o "stdout.test.%N"          # standard output, %j adds job number to output file name and %N adds the node name
#SBATCH -e "stderr.test.%N"          #optional, prints standard error





# [EDIT THE CODE BELOW] Load modules, insert code, and run programs

#module load <module_name>         # optional, uncomment the line and load
date
module purge

module load dorado

dorado summary allpod5s.fastq > summarized.tsv

module purge
date
